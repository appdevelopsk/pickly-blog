...
function collectAmazon(clicksByDomain = []) {
  ...
  // ★2026-09-14: it だけでなく退役5カ国すべてを除外。uk/fr/es/ca も asp.ts の
  //   RETIRED_AMAZON_NETWORKS で US(Earn Globally)へ退避済みで、リンクが1本も
  //   向いていない(本番実測: 退役タグの混入ゼロ)。CSV には過去行が残るので、
  //   除外しないと「UK のログインが切れている」と毎日鳴り続ける = 是正済みの
  //   残骸を現在の障害と誤読させる。DE は本承認済み(pickly01-21)なので対象外。
  const RETIRED_MARKETS = new Set(["it", "uk", "fr", "es", "ca"]);
