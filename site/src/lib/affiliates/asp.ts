 ...
 *   (DE=本承認済み・JP/US=稼働中)
 *
 * ■ 2026-08-21 IT 却下 → 退避済み / 2026-08-24 異議は通らなかった(確定)
 ...
 *   スナップショットを見たものと考えられ、異議申立ての根拠があると判断していた。
 *
 *   ★2026-08-24 amazon.it から正式回答(CASE 13199276082)。異議は認められず、
 *     "Dopo che una domanda è stata rifiutata, non può essere ripristinata"
 *     (却下されたアカウントは復元できない) と明言された。復帰の道は新規申請のみ。
 *     未払い残高は今月末から60日後に支払われる。
 *
 *   ★amazon-it をこの Set から外してはいけない。IT を再取得する必要も無い:
 *     US 集約(Earn Globally)がイタリアを含む10か国をカバーしており、イタリアの
 *     読者は tag=pickly07-20 のまま amazon.it へ自動転送され現地レートで計上される。
 *     個別タグに戻すと支払い閾値が国ごとに分散して不利になる(下の 08-22 の記述参照)。
 *     加えて日本在住では海外アソシエイトの新規作成自体ができない可能性がある。
 *
 *   2026-08-24 本番実測: /it/articles/ の実アンカー(href)は全て
 *   amazon.com + tag=pickly07-20、pickly06-21 と amazon.it は 0 件で退避は完了済み。
 */
/**
