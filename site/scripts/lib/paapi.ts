 ...
 *   - **アクセス要件は2段構え**（2026-09-09 に Creators API 画面で確認）:
 *       1) 承認されたアソシエイト/クリエイターアカウント（未承認だとアプリ作成自体が不可）
 *       2) その上で PA-API を使うには **直近30日に10件以上の適格売上**
 *     Pickly は 2026-09-09 時点で 1) が未承認。鍵の発行以前の段階にある。
 *     資格確認からアクセス付与まで最大48時間かかり、その間は
 *     「アソシエイトは対象外です」エラーが出る（48時間超で続くなら要件未達）。
