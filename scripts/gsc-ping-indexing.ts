...
function articleExistsForLocale(slug: string, locale: string): boolean {
  return fs.existsSync(path.join(OUT_DIR, locale, "articles", slug, "index.html"));
}
...
function buildUrls(slugs: string[]): string[] {
  ...
  for (const slug of slugs) {
    for (const locale of locales) {
      if (articleExistsForLocale(slug, locale)) {
