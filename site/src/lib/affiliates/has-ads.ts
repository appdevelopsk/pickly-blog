...
/** Returns true if at least one offer in the article has a real approved partnership for the
 *  locale's market. The local-Amazon search fallback does not count — it must not decide
...
export function hasApprovedAds(article: ArticleMeta, locale: string): boolean {
  ...
  return offers.some((o) => pickLink(o, market, { onlyApproved: true, allowFallback: false }) !== null);
}
