...
const FABRICATED = {
  ...
  // ★ 素の部分一致だと "فقدت"(失った、3人称女性/無関係語根)が "قدت" を含み、
  //   "غسلتين"(2回の洗濯、名詞)が "غسلت" を含むなど誤検出する(2026-08-18、17件誤検出)。
  //   アラビア語は無母音表記だと1人称と3人称女性の過去形が綴り同一になるが、
  //   語幹全体の部分一致を防ぐだけでも大半の誤検出は消える → 単語境界を必須にする。
  "ar": b("اختبرت|قِست|قست|استخدمت|ارتديت|جربت|أحصيت|قدت|نمت|غسلت|اتصلت|في اختباري|تجربتي"),
  "hi": /(मैंने .{0,30}(परीक्षण|इस्तेमाल|उपयोग|पहन|मापा|आज़मा)|मेरे परीक्षण|मेरा परीक्षण)/g,
  // ★★ タイ語は動詞が時制変化しないので、素の「ผมใช้/ฉันใช้」は "I used"（体験談＝要検出）
  //   だけでなく "I use / can I use"（読者視点のFAQ質問＝正常）にも一致してしまう。
  //   FAQ の質問文は全ロケールで一人称の読者視点で書く方針なので、en が "I use" を
  //   持たず "I used" だけなのと同じ扱いに揃える。ใช้ は過去マーカー「ได้」付きか、
  //   期間表現を伴う場合だけ体験談とみなす。
  "th": /(ผมได้ทดสอบ|ฉันได้ทดสอบ|ผมทดสอบ|ฉันทดสอบ|ผมวัด|ฉันวัด|ผมได้ใช้|ฉันได้ใช้|(?:ผม|ฉัน)ใช้[^<]{0,40}(?:มาแล้ว|มาเป็นเวลา|มาตลอด|มาหลายเดือน|มาหลายปี)|การทดสอบของผม|การทดสอบของฉัน)/g,
  // ★ "직접 테스트"は methodology の正直な否定形disclaimer
  //   ("...직접 테스트하지는 않았습니다"=直接テストはしていません)にも一致してしまう
  //   (2026-08-18、5件誤検出)。直後の否定接続「하지」が続く場合は除外する。
  "ko": /(제가 (테스트|측정|사용|착용|시도)|직접 테스트(?!하지)|저의 테스트|제 테스트)/g,
